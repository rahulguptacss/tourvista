...
import { Award, Users, Briefcase, Star, UserPlus, CalendarCheck, Smile, Plane, ShieldCheck, Trophy, Sparkles } from "lucide-react";
        ...
        <div className="bg-[#0b1d3d] bg-gradient-to-r from-[#071736] via-[#0b2046] to-[#071736] px-4 sm:px-6 py-8 md:py-10 rounded-b-[2rem] shadow-2xl">
          <div className="flex flex-wrap items-center justify-center max-w-5xl mx-auto md:divide-x md:divide-white/10">
            {stats.map((stat: StatItem, idx: number) => {
              ...
              if (lowerLabel.includes("award") || stat.icon === "Award") IconComponent = Award;
              else if (lowerLabel.includes("travel") || stat.icon === "Plane") IconComponent = UserPlus;
              else if (lowerLabel.includes("success") || stat.icon === "CalendarCheck") IconComponent = Briefcase;
              else if (lowerLabel.includes("experience") || stat.icon === "Smile") IconComponent = Sparkles;
              else IconComponent = Star;
              ...
              return (
                <div key={idx} className="flex flex-col items-center justify-center w-1/2 md:w-1/4 px-2 sm:px-4 py-4 md:py-0 group cursor-default">
                  <div className="transform transition-transform duration-500 group-hover:-translate-y-2 group-hover:scale-110 mb-2">
                    <IconComponent className="w-10 h-10 sm:w-12 sm:h-12 md:w-12 md:h-12 text-[#ffc107] stroke-[1.5] drop-shadow-[0_2px_4px_rgba(255,193,7,0.3)]" />
                  </div>
